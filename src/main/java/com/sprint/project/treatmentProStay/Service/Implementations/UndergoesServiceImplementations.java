...
public class UndergoesServiceImplementations implements UndergoesService {
    ...
    public UndergoesRequestDTO assignTreatment(UndergoesRequestDTO dto) {
    	...
    	PatientEntity patient = patientRepository.findById(dto.getPatientId())
    	        .orElseThrow(() -> new TreatmentNotFoundException("Patient not found"));

    	ProceduresEntity procedure = proceduresRepository.findById(dto.getProcedureId())
    	        .orElseThrow(() -> new TreatmentNotFoundException("Procedure not found"));

    	StayEntity stay = stayRepository.findById(dto.getStayId())
    	        .orElseThrow(() -> new TreatmentNotFoundException("Stay not found"));

    	if (!stay.getPatient().getSsn().equals(patient.getSsn())) {
    	    throw new InvalidTreatmentException("Patient mismatch with stay");
    	}
        ...
        if (undergoesRepository.existsById(id)) {
            throw new TreatmentAlreadyExistsException("Treatment already exists");
        }
